...
- **Role**: The elements are given relevant role and ARIA
  attributes to be accessible to screen readers. This includes
  whether the tooltip acts like a label rather than a
  description.
...
const TooltipAnchor = React.forwardRef(function TooltipAnchor(
  ...
  const ref = React.useMemo(
    () => mergeRefs([reference, propRef, children.ref]),
    [reference, propRef, children]
  );
