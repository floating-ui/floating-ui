...
- [Reusable tooltip component](https://codesandbox.io/s/jovial-artem-03ndp6?file=/src/App.tsx)
...
[CodeSandbox demo](https://codesandbox.io/s/jovial-artem-03ndp6?file=/src/App.tsx)
...
- Read the open state of the tooltip while rendering
  (`tooltip.open{:js}`), e.g. to change styles
- Control the tooltip manually beyond the interaction hooks,
  `tooltip.setOpen(boolean){:js}`
...
function App() {
  ...
  return (
    <>
      <TooltipAnchor state={tooltip}>My anchor</TooltipAnchor>
      <Tooltip state={tooltip}>My tooltip</Tooltip>
    </>
  );
}
```
...
This hook is used to control the state of the tooltip, accepting
configuration options. This contains the positioning and
interactions logic to be passed to the components.

### Components

The components accept the return value of `useTooltip(){:js}` as
...
- `<TooltipAnchor />{:js}` is the anchor button the tooltip is
  attached to.
- `<Tooltip />{:js}` is the tooltip element, which can contain
...
- [CodeSandbox demo](https://codesandbox.io/s/nostalgic-meadow-yq11qq?file=/src/Group.tsx)
