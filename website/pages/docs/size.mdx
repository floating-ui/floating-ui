...
```js
import {size} from '@floating-ui/solid';
```
...
1. You can use sync state, and apply using React props.
2. Or, mutate the style attribute directly.

```js
const [maxWidth, setMaxWidth] = createSignal(null);

useFloating({
  middleware: [
    size({
      apply({availableWidth, availableHeight, elements}) {
        // 1. You can set the width with a signal
...
such as `rects{:.objectKey}` and `elements{:.objectKey}`.
