...
not currently visible on screen for the user.

Note that `visibility` and `opacity` styles are not considered,
only the clipping of its ancestors (scrolling ancestors or
viewport).
