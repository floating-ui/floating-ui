...
Provides CSS transitions for a floating element, including
...
- [`useCSSTransition`](/docs/useCSSTransition#usecsstransition) —
  a low level hook that returns data that can be computed by you.
...
- `'closed'{:js}` indicates the element is unmounted from the
