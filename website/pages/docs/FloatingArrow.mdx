...
<Notice type="warning" title="Browser notice">
  In WebKit browsers, the arrow may have a gap if the floating
  element's `x` and `y` coordinates are fractional. You can work
  around this by [rounding the
  coordinates](/docs/misc#subpixel-and-accelerated-positioning)
