...
reference element along the relevant axis and has an edge align
(`'start'{:js}`/`'end'{:js}`). This is ignored if the shift()
middleware caused the floating element to shift.
