...
- `inner(){:js}` is a positioning middleware that anchors the
  reference element to an inner element inside of the floating
  element.
...
A callback that is invoked when a new offset upon the
