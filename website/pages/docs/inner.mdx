...
import {inner, useInnerOffset} from '@floating-ui/solid';
...
interface Props extends DetectOverflowOptions {
  ...
  overflowRef?: [
    get: SideObject,
    set: SetStoreFunction<SideObject>,
  ];
...
```tsx
import {createOverflowRef, inner} from '@floating-ui/solid';

function App() {
  const overflowRef = createOverflowRef();
...
```js
interface UseInnerOffsetProps extends DetectOverflowOptions {
  enabled?: boolean;
  overflowRef: React.MutableRefObject<SideObject | null>;
  scrollRef?: React.MutableRefObject<HTMLElement | null>;
  onChange: (
