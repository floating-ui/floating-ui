...
A popover is an interactive mini-dialog popup that displays
information related to an anchor element when the element is
clicked.
...
- **Dynamic anchored positioning**: The popover will remain
...
- [Reusable popover component](https://codesandbox.io/s/nostalgic-noyce-hj7jqz?file=/src/App.tsx)
...
[CodeSandbox demo](https://codesandbox.io/s/nostalgic-noyce-hj7jqz?file=/src/App.tsx)
...
This API, inspired by Ariakit, allows you to:

- Read the open state of the popover while rendering
  (`popover.open{:js}`), e.g. to change styles
- Control the popover manually beyond the interaction hooks,
  `popover.setOpen(boolean){:js}`
...
function App() {
  ...
  return (
    <>
      <PopoverAnchor state={popover}>My anchor</PopoverAnchor>
      <Popover state={popover}>
        <PopoverHeading>My popover label</PopoverHeading>
        <PopoverDescription>
          My popover description
        </PopoverDescription>
        <button onClick={() => popover.setOpen(false)}>
          Close
        </button>
      </Popover>
    </>
...
This hook is used to control the state of the popover, accepting
configuration options. This contains the positioning and
interactions logic to be passed to the components.

### Components

The components accept the return value of `usePopover(){:js}` as
their `state{:.keyword}` prop. These components read the state
...
- `<PopoverAnchor />{:js}` is the anchor button the popover is
  attached to.
- `<Popover />{:js}` is the popover element, which can contain
  any children (React nodes).
...
If you have a portalled modal popover, and you want to be able to
nest it to any depth, then you will want to use
`<FloatingTree />{:js}` and its related hooks and components.

[CodeSandbox demo](https://codesandbox.io/s/competent-bose-gov4sy?file=/src/App.tsx)

```js {3-4,22,24,32,40} /nodeId/
import {
  useFloatingNodeId,
  FloatingTree,
  FloatingNode,
...
function App() {
  const rootPopover = usePopover();
  const childPopover = usePopover();
  return (
    <FloatingTree>
      <PopoverAnchor state={rootPopover}>Root</PopoverAnchor>
      <Popover state={rootPopover}>
        <PopoverAnchor state={childPopover}>
          Nested
        </PopoverAnchor>
        <Popover state={childPopover}>Nested</Popover>
      </Popover>
    </FloatingTree>
  );
}
...
[View documentation for FloatingTree](/docs/FloatingTree).
