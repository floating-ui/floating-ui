...
The available sides are `'top'{:js}`, `'right'{:js}`,
`'bottom'{:js}`, `'left'{:js}`.

The alignments are `'center'{:js}`, `'start'{:js}`, and
`'end'{:js}`. These allow you to align the tooltip to one of the
...
}).then(({x, y, physicalSide, middlewareData}) => {
  Object.assign(tooltip.style, {
    left: `${x}px`,
    top: `${y}px`,
  });
