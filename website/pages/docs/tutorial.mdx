...
provide “anchored positioning”. Think of it like a polyfill for a
missing CSS feature. Pre-built styles are not provided, and user
interactions are only available for React users.

If you’re looking for something simple to set up, with pre-built
interactions or styles out of the box for a tooltip or popover
element, it is better to use a component library like Bootstrap,
Chakra UI, MUI, and so on. All of the mentioned libraries use
this library (formerly, Popper) under the hood.
...
Note that `top{:.function}` and `left{:.function}` are set to
`0{:js}` to minimize layout interference and ensure the floating
element does not resize after being positioned — especially if it
is large.
...
[Play on CodeSandbox if you aren't using a module bundler](https://codesandbox.io/s/floating-ui-dom-template-nzvvx?file=/src/index.js)
