...
Placing three `shiftByOnePixel{:.const}` to the middleware array
means the coordinates get shifted by 3 pixels in total:
...
beginning middleware, while `arrow{:.function}` and
...
```ts
type Reset = true | {placement?: Placement};
```
