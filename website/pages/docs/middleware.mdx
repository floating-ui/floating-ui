...
<ShowFor packages={['react', 'react-dom', 'react-native','solid']}>
...
<ShowFor packages={['react', 'react-dom', 'react-native', 'vue','solid']}>
...
A middleware is an object that has a `name{:.objectKey}` property
and a `fn{:.function}` property. The `fn{:.function}` property
...
The `options{:.objectKey}` key on a middleware object holds the
