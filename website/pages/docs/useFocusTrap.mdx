...
Whether the focus trap is "modal", such that focus cannot escape
the floating element and content outside it is unreachable.
...
useFocusTrap(context, {
  ...
  // NOTE: only use in non-modal mode.
