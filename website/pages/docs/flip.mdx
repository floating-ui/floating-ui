...
As `flip(){:js}` does not change the axis of placement due to the
ambiguity of the desired order, a `'left'{:js}` or `'right'{:js}`
placement on a small screen (like a mobile viewport), may result
in overflow.
...
The same concept applies on the reverse axes, although is less
common.
