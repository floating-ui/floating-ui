...
flip, the axis perpendicular to `side{:.key}`. By disabling this,
it will ignore overflow.
...
When an align is specified, e.g. `'top-start'{:js}` instead of
just `'top'{:js}`, this will flip to `'top-end'{:js}` if
...
middleware array. This ensures the `flipAlign{:.key}` logic can
act before `shift(){:js}`'s does.
...
`'best-fit'{:js}` will use the placement which fits best on the
checked axes. `'initial-placement'{:js}` will use the initial
...
computePosition(referenceEl, floatingEl, {
  ...
  console.log(side); // 'top' or 'bottom'
});
```
...
```js
// side can be 'top' or 'bottom'
const {side} = useFloating(reference, floating, {
  side: 'bottom',
  middleware: [flip()],
});
```
...
// Prioritize flip over shift for edge-aligned sides only.
if (align !== 'center') {
