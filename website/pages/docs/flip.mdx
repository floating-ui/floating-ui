...
checked axes, preferring a placement whose overflow can be
scrolled into view over one that gets clipped behind the
boundary's scroll origin (unless [`size`](/docs/size) is also
used, in which case the floating element can be resized to fit).
`'initialPlacement'{:js}` will use the initial `placement{:.key}`
specified.
