...
<div className="flex flex-col gap-4">
  <Chrome label="Scroll up" center scrollable className="h-96">
...
flip, the opposite axis of `mainAxis{:.objectKey}`. By disabling
...
The axis of placement is not changed by default due to the
ambiguity of the desired order — this means a `'left'{:js}` or
`'right'{:js}` placement on a small screen (like a mobile
viewport) may result in overflow.

If it's not known whether `'top'{:js}` or `'bottom'{:js}` should
be preferred on a mobile screen, then allowing the consumer of a
component to manually specify the value is a good idea.
...
computePosition(referenceEl, floatingEl, {
  ...
  middleware: [
    flip({
      // Should 'top' or 'bottom' be preferred first?
...
The same concept applies on the reverse axis, although is less
common.
