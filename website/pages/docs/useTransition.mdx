...
Provides a transitions to apply CSS transitions to a floating
element, including correct handling of "placement-aware"
transitions. If you're using a spring library like
`framer-motion` or `react-spring`, then this may not be
necessary.
...
- [`useTransitionStatus`](/docs/useTransition#useTransition) — a
  low level hook that returns a status string to compute the
  styles yourself.
...
```js
const {isMounted, status} = useTransition(context, {
  duration: 1000,
});
```
