...
interface Props {
  ...
  referencePressEvent?: MaybeAccessor<
...
outside the floating element's <ShowFor
packages={['react']}>React</ShowFor><ShowFor
packages={['solid']}>component</ShowFor> tree and don't want the
...
```js
function App() {
  // The toast is not inside the Dialog's component tree, so we
  // need to add a guard to consider it a child of the Dialog
  // to prevent the Dialog's outside press from closing it.
  return (
    <>
