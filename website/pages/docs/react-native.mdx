...
Positioning is relative to the window by default. If both
elements are in the same non-root parent or
`<ScrollView />{:jsx}`, pass `sameScrollView: true`.

See [ScrollView](/docs/react-native#scrollview) or
...
On Android, `sameScrollView: true` is currently broken due to an
