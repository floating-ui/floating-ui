...
(recommended), use the `whileElementsMounted{:.objectKey}`
option:
...
`whileElementsMounted{:.objectKey}` automatically handles calling
and cleaning up `autoUpdate` based on the presence of the
