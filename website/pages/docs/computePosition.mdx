...
The CSS styles applied **before** `computePosition(){:js}` is
called therefore have significance:
...
allowing the `top{:.objectKey}` and `left{:.objectKey}`
coodinates to take effect.
...
- `'absolute'{:js}` — the floating element is positioned relative
  to the nearest ancestor with a CSS `position{:.function}`
  property of `relative{:.param}` or `absolute{:.param}`. This is
  the default strategy. With most layouts, this usually requires
  the browser to do the least work when updating the position.
...
- [size](/docs/size) resizes the floating element so it will not
  overflow a clipping ancestor.

#### Data providers

These middleware provide data and do not alter the coordinates.
