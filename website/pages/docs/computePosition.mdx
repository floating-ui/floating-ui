...
It runs only once — [`autoUpdate`](/docs/autoUpdate) is used with
it to ensure the floating element remains anchored by continually
re-running the calculation as necessary.
...
consumer. They compose and work together to produce the final
