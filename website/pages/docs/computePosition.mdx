...
This is an async function that computes and returns x and y
coordinates to position a floating element next to a reference
element. [autoUpdate](/docs/autoUpdate) is used with it to ensure
the floating element remains "anchored" to its reference element.
...
<Notice>
  These styles are explained below in [Initial
  layout](/docs/computePosition#initial-layout).
</Notice>
...
<Notice>
  ...
  [offset](/docs/offset#creating-custom-placements) allows you to
  create _any_ placement.
</Notice>
...
- [size](/docs/size) resizes the floating element, for example so
  it will not overflow a clipping container, or to match the
