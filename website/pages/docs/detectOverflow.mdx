...
```ts
type RootBoundary = 'viewport' | 'layoutViewport' | 'document' | Rect;
```
...
`'layoutViewport'{:js}` is the
[Layout Viewport](https://developer.mozilla.org/en-US/docs/Glossary/Layout_viewport),
which remains stable while pinch-zooming or when the mobile
keyboard is open. This is useful when the floating element should
stay anchored in place rather than shift around as the visual
viewport changes, such as for context menus.

`'document'{:js}` is the entire page outside the viewport.
