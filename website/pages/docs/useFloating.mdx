...
`@floating-ui/react-dom{:.string}`; if later on you decide you
want to add interactions, you can uninstall
...
Call the hook inside a component. See
[Positioning](/docs/react#positioning) for the most basic usage.
...
instead of layout (`top{:.string}` and `left{:.string}`).
...
transform animations. In that case, you can use the floating
element as a wrapper allows you to retain CSS transforms for
positioning:
...
An array of middleware functions that change the positioning of
...
Here's an example enabling a middleware with a reactive function
option:
...
function App() {
  ...
  useFloating({
    middleware: [
      // No need to use the reactiveOffset version:
      offset(value),
...
externally passing the element(s), as an alternative to the
`refs{:.const}` object.

They should be held in state (not plain refs) to ensure that they
are reactive.
...
<Notice type="warning">
  Only use this prop if your floating element is _conditionally
...
Available only for React, this object contains the context values
provided to relevant hooks and components.
