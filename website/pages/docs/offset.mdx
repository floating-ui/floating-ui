...
placement, writing direction (e.g. RTL), or align.
...
The same axis as `align{:.key}` but applies only to aligned
placements and inverts the `end{:.string}` align. When set to a
number, it overrides the `align{:.key}` value.
...
Here's how it differentiates from `align{:.key}`:
  ...
  <div className="mt-4 flex flex-col gap-4 md:flex-row">
    ...
    >
      <div className="mx-auto grid h-32 w-48 place-items-center border-2 border-dashed border-gray-1000 text-center">
        top-start <br />{' '}
        <strong className="text-inherit">(align)</strong>
      </div>
    </Floating>
    <Floating
      placement="top-end"
...
This time, `'top'{:js}` and `'start'{:js}` were used as the
starting point.
