...
<ShowFor packages={['solid']}>
	<Notice>Currently you can only use static values for the middleware.</Notice>
</ShowFor>
<ShowFor packages={['react', 'react-dom']}>
...
const {refs} = useFloating({ elements: { reference }});
...
const {isPositioned} = useFloating({ open: isOpen });

// Once `isOpen` flips to `true`, `isPositioned` will switch to
`true` // asynchronously. We can use an effect to determine when
it has // been positioned. useEffect(() => { if (isPositioned) {
// ... } }, [isPositioned]);
...
event and reason that caused the change.
...
<ShowFor packages={['react']}>
[FloatingTree](/docs/react/FloatingTree).
</ShowFor>
<ShowFor packages={['solid']}>
[FloatingTree](/docs/solid/FloatingTree).
</ShowFor>
