...
```js
import {useListNavigation} from '@floating-ui/solid';
```
...
- See <ShowFor
  packages={['react']}>[`FloatingList`](/docs/react/FloatingList)</ShowFor><ShowFor
  packages={['solid']}>[`FloatingList`](/docs/solid/FloatingList)</ShowFor>
  for creating composable children API components.
- See <ShowFor
  packages={['react']}>[`Composite`](/docs/react/Composite)</ShowFor><ShowFor
  packages={['solid']}>[`Composite`](/docs/solid/Composite)</ShowFor>
...
interface Props {
  ...
  scrollItemIntoView?: MaybeAccessor<
...
<Notice type="error" title="Required">
  ...
  [`FloatingTree`](/docs/react/FloatingTree) setup so that
...
<ShowFor packages={["solid"]}>
<Notice title="Info">
 Currenttly not available for Solid.
</Notice>
</ShowFor>
