...
```js
import {useInteractions} from '@floating-ui/solid';
```
...
for all types of floating elements. See <ShowFor
packages={['react']}>[`useRole`](/docs/react/useRole#role)</ShowFor><ShowFor
packages={['solid']}>
[`useRole`](/docs/solid/useRole#role)</ShowFor> for more
information on this prop getter for listbox (e.g. select or
combobox) or menu roles.
