...
```js
import {useRole} from '@floating-ui/solid';
```
...
getter should be called with `active{:.objectKey}` and
`selected{:.objectKey}` boolean props when spread.
...
`selectedIndex` states for the <ShowFor
packages={['react']}>[`useListNavigation`](/docs/react/useListNavigation)</ShowFor><ShowFor
packages={['solid']}>
[`useListNavigation`](/docs/solid/useListNavigation)</ShowFor>and
<ShowFor
packages={['react']}>[`useTypeahead`](/docs/react/useTypeahead)</ShowFor><ShowFor
packages={['solid']}>
[`useTypeahead`](/docs/solid/useTypeahead)</ShowFor> hooks.
