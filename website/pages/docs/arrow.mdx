...
- The reference element is either wider or taller than the
  floating element;
- The floating element is using an edge align (`-start{:.string}`
  or `-end{:.string}` placement).
...
middleware array, after `shift(){:js}` or `autoPlacement(){:js}`
(if used).
...
computePosition(referenceEl, floatingEl, {
  ...
  // The final rendered side can be 'bottom' or 'top'
  const renderedSide = data.side;
});
```
