...
inner element of the floating element (usually a triangle or
caret) so that it is centered to the reference element.
...
Pass it to the `arrow{:.function}` middleware and assign the
dynamic styles:
...
`arrow{:.function}` should generally be placed toward the end of
your middleware array, after `shift{:.function}` (if used).
