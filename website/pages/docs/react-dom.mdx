...
`onElementsMounted{:.objectKey}` prop, which is fully reactive
and will add and remove the listeners so that it only active
while both elements are mounted:
...
function App() {
  const {x, y, reference, floating, strategy, refs} =
    useFloating({
      // default signature
      onElementsMounted: autoUpdate,

      // or, pass `autoUpdate` options.
      // make sure to return the cleanup function!
      onElementsMounted: (referenceEl, floatingEl, update) =>
...
The `onElementsMounted{:.objectKey}` callback also allows you to
react to the elements mounting/unmounting beyond using the
