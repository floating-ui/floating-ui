...
> <span style={{fontFamily: 'sans-serif'}}>⚠️</span> **Warning!** Do not pass unstable inline callbacks to the
`ref{:.keyword}` prop. This results in an infinite loop because
a state update occurs inside,
[but React calls unstable callback refs thinking it's a new function on each render](https://github.com/facebook/react/issues/6249#issuecomment-195412139).
...
// ✅ Good, memo unstable inline callbacks
const localRef = useRef();
const reference = useMemo(
  () => mergeRefs([floatingReference, localRef]),
  [floatingReference]
);
