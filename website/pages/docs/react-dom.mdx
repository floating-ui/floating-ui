...
The `useFloating()` hook is generic, to allow specifying a
stronger reference type. By default, the reference type is wide,
allowing `Element | VirtualElement | null`. This can cause issues
in TypeScript, if you want to reuse the refs returned by the
hook, as `VirtualElement` does not have most of the native
methods on elements.

For example, you couldn't check if the reference element contains
another node. To fix this, you can pass a generic argument to the
hook the specify a more specific type for the reference:
  ...
  useEffect(() => {
    if (
      refs.reference.current?.contains(
        document.querySelector('#another-element')
      )
    ) {
      // All good!
      // do something
