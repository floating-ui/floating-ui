...
The `ref{:.keyword}` prop values `<div ref={reference} />{:js}`
and `<div ref={floating} />{:js}` must be stable references,
otherwise an infinite loop will occur.

If you want to merge another ref with it, utilize the
`useMemo(){:js}` hook (ensuring both of the refs are also
stable):
...
> While `useMemo(){:js}` isn't a semantic guarantee, as long as
> React doesn't "forget" the memo'd value in an endless loop,
> this method is fine.
