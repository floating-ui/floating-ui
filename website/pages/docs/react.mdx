...
- `refs{:.const}` contains setter function refs that ensure the
  position is updated when either element changes (e.g.
...
By default, the `reference{:.const}` element is **both** the
events and position reference. The `positionReference{:.const}`
callback ref allows you to separate the position to another
element (either real or virtual):
