...
primitives to build floating UI components. This allows you to
create components such as tooltips, popovers, dropdown menus,
hover cards, modal dialogs, select menus, comboboxes, and more.

**Note that this library is intentionally low-level and provides
primitives, rather than pre-built components — this means you use
the library build your own components**. This makes it flexible,
but more complex to use. If you're looking for pre-built
components, you will likely find other libraries better suited
for your use case.
...
the following package is smaller in size. The above is superset
...
Both the Positioning and Interactions sections should be read in
order before reading the API docs for the hooks and components in
the left navigation bar (`useHover`, `useFocus`, etc.)
