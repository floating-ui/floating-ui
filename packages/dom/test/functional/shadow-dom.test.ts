    ...
    ['static', 'relative', 'absolute'].forEach((cssPosition) => {
      ['absolute', 'fixed'].forEach((strategy) => {
        [true, false].forEach((withTransform) => {
          test(`shadow DOM (${useCase}): correctly positioned on ${strategy} + host-position-${cssPosition} + ${placement} + ${withTransform ? "with" : "without"}`, async ({ page }) => {
            ...
            expect(await page.locator('.container').screenshot()).toMatchSnapshot(
              `${useCase}-host-position-${cssPosition}-${placement}-${strategy}-${withTransform ? "with" : "without"}-transform.png`,
            );
          });
        });
      });
