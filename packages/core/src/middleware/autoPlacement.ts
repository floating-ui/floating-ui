...
export function* autoPlacementGen(
  ...
  const {rects, middlewareData, side, align, platform, elements} = state;
  ...
  if (side !== currentPlacement.side || align !== currentPlacement.align) {
    return {
      reset: {
  ...
  if (resetPlacement.side !== side || resetPlacement.align !== align) {
    return {
      data: {
        index: currentIndex + 1,
        overflows: allOverflows,
