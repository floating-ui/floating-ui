...
export function* arrowGen(
  ...
  const arrowOffsetParent = yield platform.getOffsetParent?.(element);
  let clientSize = arrowOffsetParent ? arrowOffsetParent[clientProp] : 0;

  // DOM platform can return window as the offsetParent.
  if (!clientSize || !(yield platform.isElement?.(arrowOffsetParent))) {
